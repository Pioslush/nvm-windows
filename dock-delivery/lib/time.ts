 ...
 * facility's timezone, always with an explicit zone label (e.g. "2:30 PM MT").
 */
...
export function zoneAbbr(timezone: string, at: Date = new Date()): string {
  ...
  // Collapse MST/MDT → MT etc. for the dock-sign look.
...
/** A local date ("2026-03-14") + local time ("14:30") in a facility timezone → UTC Date. */
export function facilityLocalToUtc(dateStr: string, timeStr: string, timezone: string): Date {
  return fromZonedTime(`${dateStr}T${timeStr}:00`, timezone);
}

/** Today's date string (yyyy-MM-dd) in a facility's timezone. */
export function todayInFacility(timezone: string): string {
