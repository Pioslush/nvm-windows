...
import { cancelBooking, markRunningLate } from "@/app/actions/carrier";
import { formatDayLong, formatTimeRange, hoursUntil } from "@/lib/time";
import type { CarrierBooking } from "./page";
...
export default function BookingCard({ booking }: { booking: CarrierBooking }) {
  const [cancelState, cancelAction, cancelPending] = useActionState(cancelBooking, null);
  const facility = booking.slot.dock.facility;
  const chip = STATUS_CHIP[booking.status];
  const withinCutoff = hoursUntil(booking.slot.start_time) < facility.cancellation_cutoff_hours;
  const isToday = hoursUntil(booking.slot.start_time) < 24;
    ...
    <div className="card">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div>
          ...
          <p className="text-slate-600">{facility.city}</p>
        </div>
      ...
      <div className="mt-3 rounded-lg bg-slate-50 p-4">
        <p className="text-xl font-extrabold text-slate-900">
          {formatTimeRange(booking.slot.start_time, booking.slot.end_time, facility.timezone)}
        </p>
        <p className="mt-0.5 font-semibold text-slate-700">
          {formatDayLong(booking.slot.start_time, facility.timezone)} · {booking.slot.dock.name}
        </p>
        ...
        <p className="mt-2 text-sm text-slate-500">&ldquo;{booking.purpose}&rdquo;</p>
      </div>

      {booking.status !== "cancelled" && booking.status !== "no_show" && (
        <div className="mt-4 flex flex-wrap gap-2">
          {isToday && booking.status !== "late" && booking.status !== "checked_in" && (
          ...
          {booking.status === "late" && (
            <p className="w-full rounded-lg bg-orange-50 px-4 py-3 text-sm font-semibold text-orange-800">
              The facility has been notified you&apos;re running late. Head to {booking.slot.dock.name} when you arrive.
            </p>
          )}
          {!withinCutoff && (
            <>
              <Link
                href={`/carrier/facilities/${facility.id}?reschedule=${booking.id}`}
          ...
          {withinCutoff && !isToday && (
            <p className="w-full text-sm text-slate-500">
              Changes are locked within {facility.cancellation_cutoff_hours} hours of your slot — call the facility if plans change.
            </p>
          )}
        </div>
      )}
