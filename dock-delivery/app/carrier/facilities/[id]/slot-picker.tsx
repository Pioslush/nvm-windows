...
import { bookSlot, rescheduleBooking } from "@/app/actions/carrier";
...
export default function SlotPicker({
  ...
  const [activeDate, setActiveDate] = useState<string | null>(dateGroups[0]?.date ?? null);
  ...
  const totalOpen = dateGroups.reduce((n, d) => n + d.dockGroups.reduce((m, g) => m + g.slots.length, 0), 0);

  if (totalOpen === 0) {
    return (
      <div className="card mt-6 text-center">
        <p className="text-3xl">😕</p>
        <h2 className="mt-2 text-lg font-bold text-slate-900">No open slots in the next two weeks</h2>
        <p className="mt-1 text-slate-600">
          Check back later, or contact the facility directly.
        </p>
      </div>
    );
  }

  const currentGroup = dateGroups.find((d) => d.date === activeDate) ?? dateGroups[0];
  ...
  return (
    <form action={rescheduleBookingId ? reschedAction : bookAction} className="mt-6 flex flex-col gap-6">
      ...
      <p className="text-sm font-semibold text-slate-500">
        All times {timezoneLabel}. Pick a date, then tap a slot to select it.
      </p>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1">
        {dateGroups.map((group) => (
          <button
            ...
            className={`shrink-0 rounded-lg border-2 px-4 py-2 font-semibold ${
              currentGroup?.date === group.date
      ...
      {currentGroup?.dockGroups.map((group) => (
      ...
      {!rescheduleBookingId && (
        <div>
          ...
          <textarea
            ...
            rows={2}
            className="field"
            placeholder="Pallet drop — 12 pallets, 1 box truck"
          />
        </div>
      )}
