...
describe.runIf(!!DATABASE_URL)("double-booking protection", () => {
  ...
  beforeAll(async () => {
    ...
    const facility = await pool.query(
      `insert into facilities (name, city, timezone, created_by) values ('Test Distribution Center','Testville','America/Denver',$1) returning id`,
      [adminId]
    );
    const dock = await pool.query(
      `insert into docks (facility_id, name) values ($1, 'Dock A — North') returning id`,
      [facility.rows[0].id]
    );
    dockId = dock.rows[0].id;
    const slot = await pool.query(
      `insert into slots (dock_id, start_time, end_time)
       values ($1, now() + interval '20 hours', now() + interval '20.5 hours') returning id`,
      [dockId]
    );
    slotId = slot.rows[0].id;

    const mkCarrier = async (email: string, company: string) => {
    ...
    carrierA = await mkCarrier("a@test.local", "Alpha Freight");
    carrierB = await mkCarrier("b@test.local", "Bravo Logistics");
  });
  ...
  const book = (carrierId: string, slot: string) =>
    pool.query(
      `insert into bookings (slot_id, carrier_id, purpose) values ($1, $2, 'test delivery') returning id`,
      [slot, carrierId]
    );

  it("two simultaneous bookings for the same slot: exactly one succeeds", async () => {
    const results = await Promise.allSettled([book(carrierA, slotId), book(carrierB, slotId)]);
  ...
  it("a third attempt on the booked slot also fails", async () => {
    await expect(book(carrierB, slotId)).rejects.toMatchObject({ code: "23505" });
  });

  it("cancelling reopens the slot and lets another carrier book it", async () => {
    ...
    const rebook = await book(carrierB, slotId);
  ...
  it("cannot book a blocked slot", async () => {
    const blocked = await pool.query(
      `insert into slots (dock_id, start_time, end_time, status)
       values ($1, now() + interval '21 hours', now() + interval '21.5 hours', 'blocked') returning id`,
      [dockId]
    );
    await expect(book(carrierA, blocked.rows[0].id)).rejects.toThrow(/blocked/);
  });

  it("cannot book a slot in the past", async () => {
    const pastSlot = await pool.query(
      `insert into slots (dock_id, start_time, end_time)
       values ($1, now() - interval '2 hours', now() - interval '1.5 hours') returning id`,
      [dockId]
    );
    await expect(book(carrierA, pastSlot.rows[0].id)).rejects.toThrow(/in the past/);
  });

  it("50 concurrent bookings for one slot: exactly one wins", async () => {
    const slot = await pool.query(
      `insert into slots (dock_id, start_time, end_time)
       values ($1, now() + interval '23 hours', now() + interval '23.5 hours') returning id`,
      [dockId]
    );
    const stampede = await Promise.allSettled(
      Array.from({ length: 50 }, (_, i) => book(i % 2 === 0 ? carrierA : carrierB, slot.rows[0].id))
    );
